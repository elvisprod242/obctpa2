...
import { Loader2, Edit, Printer, Clock } from 'lucide-react';
...
function TempsTravailForm({
  ...
  const [formValues, setFormValues] = useState<TempsTravailFormValues>(tempsTravail || {
    analyse_cause: '',
    action_prise: '',
    suivi: '',
  });

  const handleChange = (field: keyof TempsTravailFormValues, value: string) => {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
        <div className="space-y-2">
            ...
            <Input id="analyse_cause" placeholder="Trafic dense" value={formValues.analyse_cause} onChange={e => handleChange('analyse_cause', e.target.value)} />
        </div>
        <div className="space-y-2">
            <Label htmlFor="action_prise">Action prise</Label>
            <Input id="action_prise" placeholder="Optimisation du planning" value={formValues.action_prise} onChange={e => handleChange('action_prise', e.target.value)} />
        </div>
        <div className="space-y-2">
            <Label htmlFor="suivi">Suivi</Label>
            <Input id="suivi" placeholder="Amélioration constatée" value={formValues.suivi} onChange={e => handleChange('suivi', e.target.value)} />
        </div>
        <DialogFooter>
...
export default function TempsTravailPage() {
  ...
  const { weeklyData: driverAnalysisData, monthlyTotalSeconds } = useMemo(() => {
    if (!rapports || !selectedDriverId || !objectifs || !tempsTravailData || !invariants || !activePartner) return { weeklyData: [], monthlyTotalSeconds: 0 };
    ...
    for (const report of driverMonthlyReports) {
      try {
        ...
        const reportSeconds = parseTimeString(report.duree);
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">
                    Total Temps de Travail Mensuel
                  </CardTitle>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Total des heures de travail pour la période sélectionnée.
                  </p>
                </CardContent>
              </Card>
